...
public sealed class ScheduleRepository(ScheduleDbContext db) : IScheduleRepository
{
    ...
    {
        IQueryable<SchedulePlan> query = db.Schedules.AsSplitQuery().Include(x => x.Faculty).Include(x => x.Groups).Include(x => x.Subjects).Include(x => x.Lecturers).Include(x => x.SubjectLecturers).Include(x => x.Notes).Include(x => x.Entries).ThenInclude(x => x.EntryGroups).Include(x => x.Entries).ThenInclude(x => x.Comments);
    ...
    public Task<ScheduleNote?> GetNoteAsync(Guid id, CancellationToken ct) => db.ScheduleNotes.Include(x => x.Schedule).FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null, ct);
