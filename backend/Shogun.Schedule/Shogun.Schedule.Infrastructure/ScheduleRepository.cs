...
public sealed class ScheduleRepository(ScheduleDbContext db) : IScheduleRepository
{
    ...
    {
        IQueryable<SchedulePlan> query = db.Schedules.AsSplitQuery().Include(x => x.Faculty).Include(x => x.Groups).Include(x => x.Subjects).Include(x => x.Lecturers).Include(x => x.SubjectLecturers).Include(x => x.Notes).ThenInclude(x => x.Recipients).Include(x => x.Entries).ThenInclude(x => x.EntryGroups).Include(x => x.Entries).ThenInclude(x => x.Comments).ThenInclude(x => x.Recipients);
    ...
    public async Task AddSubjectLecturerAsync(ScheduleSubjectLecturer item, CancellationToken ct) => await db.ScheduleSubjectLecturers.AddAsync(item, ct);
    public async Task AddNoteAsync(ScheduleNote note, CancellationToken ct) => await db.ScheduleNotes.AddAsync(note, ct);
    ...
    public Task<ScheduleComment?> GetCommentAsync(Guid id, CancellationToken ct) => db.ScheduleComments.Include(x => x.Recipients).Include(x => x.ScheduleEntry).ThenInclude(x => x.Schedule).FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null, ct);
    public Task<ScheduleEntry?> GetEntryAsync(Guid id, CancellationToken ct) => db.ScheduleEntries.Include(x => x.Schedule).Include(x => x.Comments).ThenInclude(x => x.Recipients).FirstOrDefaultAsync(x => x.Id == id, ct);
    public Task<ScheduleNote?> GetNoteAsync(Guid id, CancellationToken ct) => db.ScheduleNotes.Include(x => x.Recipients).Include(x => x.Schedule).FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null, ct);
