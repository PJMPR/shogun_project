...
public sealed class ScheduleRepository(ScheduleDbContext db) : IScheduleRepository
{
    ...
    {
        IQueryable<SchedulePlan> query = db.Schedules.Include(x => x.Faculty).Include(x => x.Groups).Include(x => x.Entries).ThenInclude(x => x.EntryGroups).Include(x => x.Entries).ThenInclude(x => x.Comments);
