...
public sealed class ScheduleController(IScheduleService service) : ControllerBase
{
    ...
    [HttpDelete("{id:guid}/lecturers/{lecturerId:guid}"), Authorize(Roles = "admin,planner")] public async Task<IActionResult> DeleteLecturer(Guid id, Guid lecturerId, CancellationToken ct) { await service.DeleteLecturerAsync(id, lecturerId, ct); return NoContent(); }
