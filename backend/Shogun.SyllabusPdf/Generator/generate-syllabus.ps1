# generate-syllabus.ps1
# Generuje jeden PDF sylabusu z pliku JSON.
# Uzycie:
#   .\generate-syllabus.ps1 -InputFile .\ASD.json -OutputFile .\ASD.pdf
...
if (-not (Test-Path -LiteralPath $InputFile -PathType Leaf)) {
    throw "Nie znaleziono pliku wejsciowego: $InputFile"
}

# Wykryj lokalizację MiKTeX: najpierw sprawdź PATH, potem profil użytkownika
$PDFLATEX_COMMAND = $null
$pdflatexInPath = Get-Command pdflatex -ErrorAction SilentlyContinue
if ($pdflatexInPath) {
    $PDFLATEX_COMMAND = $pdflatexInPath.Source
} elseif ($env:OS -eq "Windows_NT") {
    $candidate = "$env:USERPROFILE\AppData\Local\Programs\MiKTeX\miktex\bin\x64"
    if (Test-Path "$candidate\pdflatex.exe") {
        $PDFLATEX_COMMAND = "$candidate\pdflatex.exe"
    }
}
if (-not $PDFLATEX_COMMAND) {
    throw "Nie znaleziono pdflatex. Zainstaluj MiKTeX/TeX Live lub dodaj pdflatex do PATH."
}

$SCRIPT_DIR = $PSScriptRoot
$ASSETS_DIR = Join-Path $SCRIPT_DIR "assets"
$TEMP_DIR   = Join-Path ([System.IO.Path]::GetTempPath()) ("shogun-syllabus-pdf-" + [guid]::NewGuid().ToString("N"))
...
function Compile-Tex([string]$texPath, [string]$outputFile) {
    $name = [System.IO.Path]::GetFileNameWithoutExtension($texPath)
    for ($i = 1; $i -le 3; $i++) {
        $previousErrorActionPreference = $ErrorActionPreference
        $ErrorActionPreference = "Continue"
        $result = & $PDFLATEX_COMMAND `
            -interaction=nonstopmode `
            -halt-on-error `
            -no-shell-escape `
            -output-directory="$TEMP_DIR" `
            "$texPath" 2>&1
        $pdflatexExitCode = $LASTEXITCODE
        $ErrorActionPreference = $previousErrorActionPreference
        if ($pdflatexExitCode -ne 0) {
            Write-Host "  BLAD ($name, przebieg $i)" -ForegroundColor Red
            $result | Select-Object -Last 40 | ForEach-Object { Write-Host "    $_" -ForegroundColor Yellow }
            return $false
        }
    }
    $srcPdf = Join-Path $TEMP_DIR "$name.pdf"
    $dstPdf = $outputFile
    if (Test-Path $srcPdf) {
        $outputDirectory = Split-Path -Parent $dstPdf
        if ($outputDirectory) {
            New-Item -ItemType Directory -Force -Path $outputDirectory | Out-Null
        }
        Copy-Item $srcPdf $dstPdf -Force
        $size = [math]::Round((Get-Item $dstPdf).Length / 1KB, 1)
        Write-Host "OK: $dstPdf ($size KB)" -ForegroundColor Green
        return $true
    }
    return $false
}
...
try {
    ...
    if ([System.IO.Path]::GetExtension($resolvedOutput) -ne ".pdf") {
        throw "Plik wyjsciowy musi miec rozszerzenie .pdf: $resolvedOutput"
    }
    ...
    if (-not $json.sylabus) {
        throw "Dokument JSON nie zawiera wymaganego obiektu 'sylabus'."
    }

    $s = $json.sylabus
    $code = [string]$s.kod_przedmiotu
    if ([string]::IsNullOrWhiteSpace($code)) {
        throw "Pole 'sylabus.kod_przedmiotu' jest wymagane."
    }

    New-Item -ItemType Directory -Force -Path $TEMP_DIR | Out-Null
    $tex = Generate-Tex $s $code
    $safeFileName = [System.IO.Path]::GetFileNameWithoutExtension($resolvedOutput)
    $texPath = Join-Path $TEMP_DIR "$safeFileName.tex"
    [System.IO.File]::WriteAllText($texPath, $tex, [System.Text.UTF8Encoding]::new($false))

    if (-not (Compile-Tex $texPath $resolvedOutput)) {
        throw "Nie utworzono pliku PDF."
    }
} finally {
    if (Test-Path -LiteralPath $TEMP_DIR) {
