/**
 * generate-tex-nstac.mjs  –  styl identyczny z sylabusami PJATK
 *  – wersja: STARY program niestacjonarny (public/assets/stary/nstac)
 */
...
const BASE  = `${__dir}/../../../frontend/public/assets/stary/nstac`;
  ...
  items.forEach((item, i) => {
    ...
    const prk  = item.mnsiw
      ? [...new Map(item.mnsiw.map(m => [m.kod + (m.inzynierskie ? '_inz' : ''), m])).values()]
          ...
          .join(', ')
      : '';
    out += `${bg}${esc(kod)} & ${esc(item.tresc)} & {\\scriptsize ${prk}} \\\\\n`;
  });
  out += `\\bottomrule\n\\end{longtable}}\n\n`;
  return out;
}
  ...
  for (const sem of semesters) {
    ...
    for (const subj of sem.subjects) {
      ...
      if (subj.type === 'O' && grpId && !printed.has(grpId)) {
        ...
        // etykieta: grp.label zawiera pełną nazwę grupy (z electives-other.json)
        const rawLbl = grp ? grp.label : subj.name;
        const lbl    = esc(rawLbl);
...
function buildDocument(tryb, semesters, elSpec, elOth, ef) {
  ...
  // ścieżki z folderu stare/latex-studia/ do zasobów w latex/
...
// ── Zapis ──────────────────────────────────────────────────────────────────────
const texN = buildDocument('niestacjonarny', programS.semesters, elSpecS, elOthS, efekty);
writeFileSync(`${__dir}/program_niestacjonarny_stary.tex`, texN, 'utf8');
process.stdout.write('OK: program_niestacjonarny_stary.tex\n');
