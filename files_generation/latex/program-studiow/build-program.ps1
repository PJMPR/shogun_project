...
# 4) Kopiuj PDF-y do frontend/public/assets/files
$publicFilesDir = Join-Path $root '..\..\..\frontend\public\assets\files'
New-Item -ItemType Directory -Force -Path $publicFilesDir | Out-Null

Write-Host "`n==> Kopiowanie PDF-ow do frontend\public\assets\files..." -ForegroundColor Cyan
$copied = 0
Get-ChildItem (Join-Path $outDir '*.pdf') -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    Write-Host "    OK: $($_.Name) ($size KB)  ->  frontend\public\assets\files\" -ForegroundColor Green
    $copied++
}
if ($copied -eq 0) {
