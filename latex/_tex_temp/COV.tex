...
\endhead
  \hline
  Podstawowe operacje przetwarzania obrazu. Zapoznanie z biblioteką OpenCV. & & \\
  \hline
  Podstawy działania sieci splotowej. & & \\
  \hline
  Zaawansowane architektury bazujące na sieciach splotowych (LeNet, AlexNet, VGG, Inception). & & \\
  \hline
  Zaawansowane architektury bazujące na sieciach splotowych (ResNet, DenseNet, MobileNet, SqueezeNet, EfficientNet). Transfer Learning. & & \\
  \hline
  Detekcja obrazu. Architektura modeli typu YOLO. & & \\
  \hline
  Segmentacja obrazu. Metryki do segmentacji oraz najbardziej znane architektury. & & \\
  \hline
  Autoencoder oraz Variation Autoencoder. & & \\
  \hline
  ...
  Stable Diffusion. & & \\
  \hline
  Segment Anything oraz DINO. & & \\
  \hline
  Generowanie obrazu. Style Transfer. & & \\
  \hline
  Rekonstrukcja 3D na podstawie obrazu 2D. & & \\
  \hline
  Detekcja pozy. Rozpoznawanie twarzy. & & \\
  \hline
\end{longtable}
